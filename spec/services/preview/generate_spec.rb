  ...
  describe ".call" do
    context "PDF generation" do
      ...
      context "when ActiveStorage previewer is available" do
        it "uses ActiveStorage previewer and sizes to 600x800" do
          ...
          allow(document.file.blob).to receive(:previewable?).and_return(true)
          ...
          expect(document.preview_image).to be_attached
        end
      end

      context "when ActiveStorage previewer is not available" do
        before do
          allow(document.file).to receive(:variable?).and_return(false)
          allow(document.file.blob).to receive(:previewable?).and_return(false)
        end
    ...
    context "EPUB generation" do
      it "processes EPUB with cover without error" do
        ...
        bytes, _mime = Preview::EpubCover.extract(document.file)
      ...
      it "does not attach preview when EPUB has no cover" do
        ...
        expect(document.preview_image).not_to be_attached
      end
    end
